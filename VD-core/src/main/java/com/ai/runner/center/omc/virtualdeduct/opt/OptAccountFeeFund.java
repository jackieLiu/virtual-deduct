...
import com.ai.runner.center.omc.virtualdeduct.interfaces.IOptAccountFeeFund;
...
public final class OptAccountFeeFund implements IOptAccountFeeFund {
	private static Logger logger = LoggerFactory.getLogger(OptAccountFeeFund.class);
	...
	//账本类型
	private Map<Long, SubjectFund> subjectFundMap;
	//抵扣关系
	private Map<Long,List<FunSettleRule>> fundSettleRuleMap;
	//账户未销账总述
	...
	public OptAccountFeeFund(OmcObj owner,Cycle cyclepara,Double extbalance) {
		super();
		this.owner = owner;
	...
	private void processOwn(){
		...
		Iterator<FundBook> iterator = fundBooks.iterator();
		
		while (iterator.hasNext()) {
			Fund fundBook = convert((FundBook) iterator.next());
	...
	* @throws
	...
	private RealTimeBalance caclRealInfo(){
		...
		//当前账期
		realTimeBalance.setAcctmonth(cycle.getCurrCycle());
		logger.info("OptAccountFeeFund类的caclRealInfo的accAccountInfo"+accAccountInfo.toString());
		//欠费月数
		...
		Long sumBalance = 0L;
		List<FundBook> fundBooks = fundInfo.getFundBooks();
		logger.info("OptAccountFeeFund类的caclRealInfo的fundBooks:"+fundBooks.size());
		...
		logger.info("当前余额sumBalance"+sumBalance);
		...
		for(String key:realChargesMap.keySet()){
			...
			if (charges != null){
				for (Iterator<AccChargeInfo> iterator = charges.iterator(); iterator.hasNext();) {
					...
					logger.info("OptAccountFeeFund类的caclRealInfo的AccChargeInfo:"+charge.toString());

					sumRealBill = sumRealBill.add(charge.getTotal())  ;
				}
			}
		}
	...
	private void processRealCharge(){
		...
		while (iterator.hasNext()) {
			...
			for(String key:realChargesMap.keySet()){
				...
				for (Iterator<AccChargeInfo> itcharge = charges.iterator(); itcharge.hasNext();) {
					AccChargeInfo charge = (AccChargeInfo) itcharge.next();
					if (charge.getTotal().signum()==0){
						continue;
					}
					//进行抵扣
					BwoParaForFundFee bwoParaForFundFee =  deduction(fundBook,charge);
					if (bwoParaForFundFee.getDeducteD()){
						charge.setBalance(charge.getTotal().subtract( bwoParaForFundFee.getDeductecharge()));
						fundBook.setBalance(fundBook.getBalance().subtract(bwoParaForFundFee.getDeductecharge()));
						sumdeduct = sumdeduct.add(bwoParaForFundFee.getDeductecharge()) ;
	...
	* @throws
	...
	private BwoParaForFundFee deduction(Fund fundBook,AccChargeInfo charge){
		...
		//3.比较科目
		if (isCanUse){
			SubjectFund subjectFund = subjectFundMap.get(fundBook.getSubjectId());
			
			if (subjectFund.getCanSettleAll().equals("1")){
				 List<FunSettleRule> funSettleRules = fundSettleRuleMap.get(fundBook.getSubjectId());
				 if ((funSettleRules == null)||(funSettleRules.isEmpty())){
					 isCanUse = false;
				 }else{
					 Boolean bFind = false;
					 for (Iterator<FunSettleRule> it = funSettleRules.iterator(); it.hasNext();) {
						FunSettleRule funSettleRule = (FunSettleRule) it.next();
	...
	@Override
	public void addFundinfo(FundInfo fundInfo,Map<Long, SubjectFund> subjectFund,Map<Long, List<FunSettleRule>> funSettleRule){
	...
	private Fund convert(FundBook fundBook){
		Fund fund = new Fund();
